-- VictoryRevConnect Boaters — Eval Test Boat Seed
--
-- Creates one test boat per supported model for the eval test user.
-- Run AFTER creating a dedicated eval user in Supabase Auth.
--
-- Usage:
--   1. Create an eval user via Supabase Dashboard → Auth → Users → Invite user
--      Email: eval@victoryrevconnect.internal  (do not use a real address)
--   2. Copy the new user's UUID and replace EVAL_USER_UUID below
--   3. Paste this SQL into Supabase Dashboard → SQL Editor → Run
--   4. Copy the returned UUIDs into eval/runner.ts BOAT_ID_MAP
...
-- After running, retrieve the created boat IDs:
